...
data "aws_iam_policy_document" "presigned-url-kms-iam-policy" {
  ...
  statement {
    ...
    effect = "Allow"
    principals {
      identifiers = ["${data.aws_caller_identity.current.id}", var.databricks_aws_role]
      type        = "AWS"
    }
    actions = [
      "kms:Encrypt",
...
data "aws_iam_policy_document" "general-purpose-bucket-policy" {
  statement {
    sid = "BEF General purpose Bucket Policy"
    principals {
      identifiers = [
        "${data.aws_caller_identity.current.id}"
      ]
    ...
    resources = [
      "arn:aws:s3:::${local.general-purpose-storage}",
      "arn:aws:s3:::${local.general-purpose-storage}/*"
    ]
  }
...
data "aws_iam_policy_document" "crr_s3_replication_policy" {
  ...
  statement {
    ...
    resources = [
      module.bef-presigned-url-kms[0].arn
    ]
  }
