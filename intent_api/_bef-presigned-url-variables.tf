...
variable "bef-callback-lambda_memory_size" {
  description = "How much ram to allocate for the lambda"
  type        = number
  default     = 256
}

variable "bef-callback-lambda_timeout_seconds" {
  description = "How much time until the invocation times out"
  type        = number
  default     = 30
}
...
variable "bef-callback-lambda-handler-function" {
  type        = string
  description = "Lambda function entry point"
  default     = "lambda_function.handler"
}

variable "bef-callback-lambda-runtime" {
  type        = string
  description = "Runtime config"
  default     = "python3.12"
}
...
variable "callback-url" {
  type        = string
  description = "API URL for callback"
}

variable "token-service-url" {
  type        = string
  description = "API URL for token service"
}

variable "bef-callback-lambda-function-name" {
  type        = string
  description = "Lambda function name"
  default     = ""
  validation {
    condition     = var.bef-callback-lambda-function-name != ""
    error_message = "Lambda function name required."
  }
}
...
variable "intent_consumer_key" {
  type        = string
  description = "Consumer Key for invoking Intent API"
  sensitive   = true
  validation {
    condition     = var.intent_consumer_key != ""
    error_message = "Consumer Key Not set, cannot proceed"
  }
}

variable "intent_consumer_secret" {
  type        = string
  description = "Consumer Secret for invoking Intent API"
  sensitive   = true
  validation {
    condition     = var.intent_consumer_secret != ""
    error_message = "Consumer Secret Not set, cannot proceed"
  }
}

variable "intent_consumer_secret_name" {
  type        = string
  description = "Consumer Secret for invoking Intent API"
  default     = "not set"
}

variable "eventbridge_notification_name" {
  type        = string
  description = "Notification name of eventbridge triggered by S3 file upload"
}
